namespace game {
    export class LookMatchView extends eui.Component {
        ...
        protected createChildren() {
            ...
            for (let i = 0; i < 3; i++) {
                this[`mAnimRole${i}`] = XDFFrame.DBFactory.createAnim(`db_role_${i}`, 5);
                this[`mAnimRole${i}`].setProtery({ x: 1330, y: 800, parent: this[`kGrpRole${i}`], scaleX: 0.65, scaleY: 0.65 });
            }
        ...
        public getTargetPoint(x: number, y: number): number {
            if (this.mCurSelectIdx == -1) {
                ...
                // 检测的是终点
                for (let i = 0; i < 3; i++) {
                    if (x >= this[`kGrpTar${i}`].x && x <= this[`kGrpTar${i}`].x + this[`kGrpTar${i}`].width && y >= this[`kGrpTar${i}`].y && y <= this[`kGrpTar${i}`].y + this[`kGrpTar${i}`].width) {
                        return i;
                    }
                }
            }
        ...
        private next(): void {
            if (this.mHintOrder.length <= 0) {
                ...
                XDFSoundManager.play(`sound_${this.mCurHint}_mp3`, 0, 1, 1, `sound_${this.mCurHint}_mp3`, () => {
                    this.mIsLock = false;
                });
                this[`mAnimRole${this.mCurHint}`].play(null, 2);
                this.onReset();
